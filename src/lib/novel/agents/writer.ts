...
7. **禁止脑补场景细节**：正文中出现的每个场景、地点、物品、伤势、怪物、能力，必须能在「本段事件日志」「当前场景描述」或「人物基础设定库」里找到依据。事件日志里没有出现的裂缝、残片、武器、伤口、血迹、废墟、怪形，一律不得写入正文；宁可少写一个意象，也不要无中生有。开场只能从事件日志第一条的状态写起，不允许凭空把读者带到剧情中段的某个场面。
...
- 所有具体意象必须来自本段事件日志或人物档案：事件里没有的裂缝、残片、伤口、血迹、武器、怪形、废墟，一律不能写；开场从事件日志第一条的状态写起，不许凭空跳进中段场面
